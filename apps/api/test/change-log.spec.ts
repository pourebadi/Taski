...
beforeAll(async () => {
  ...
  await prisma.user.create({ data: { id: 'lead', organizationId: ORG, fullName: 'سرپرست', username: 'lead', passwordHash: 'h', role: 'TEAM_LEAD', status: 'ACTIVE' } });
  await prisma.user.create({ data: { id: 'dev', organizationId: ORG, fullName: 'مجری', username: 'dev', passwordHash: 'h', role: 'CONTRIBUTOR', status: 'ACTIVE' } });
